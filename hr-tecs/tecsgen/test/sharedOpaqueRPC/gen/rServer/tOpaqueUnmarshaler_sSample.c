...
/* アンマーシャラ関数のプロトタイプ宣言 */
static ER  tOpaqueUnmarshaler_sSample_func(CELLCB *p_cellcb, int16_t *state);	/* func_id: 1 */
...
#endif
	switch( func_id_ ){
	case FUNCID_SSAMPLE_FUNC:		/* (id of 'func') = 1 */ 
		ercd_ = tOpaqueUnmarshaler_sSample_func( p_cellcb, &state_ );
		break;
	default:
 ...
 */
static ER
tOpaqueUnmarshaler_sSample_func(CELLCB *p_cellcb, int16_t *state_)		
{
	ER      ercd_;
