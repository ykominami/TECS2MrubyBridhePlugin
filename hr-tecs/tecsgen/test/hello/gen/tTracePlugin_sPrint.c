 ...
 *
 * 呼び口関数 #_TCPF_#
 * call port: cCall signature: sPrint context:task
 *   void           cCall_print( const char_t* str );
...
/* #[<ENTRY_FUNC>]# eThroughEntry_print
 * name:         eThroughEntry_print
 * global_name:  tTracePlugin_sPrint_eThroughEntry_print
 * oneway:       false
 * #[</ENTRY_FUNC>]# */
void
eThroughEntry_print(CELLIDX idx, const char_t* str)
{
	SYSUTM	utime;
	tTracePlugin_sPrint_CB *p_cellcb;
	if( VALID_IDX( idx ) ){
		p_cellcb = tTracePlugin_sPrint_GET_CELLCB(idx);
	}else{
		/* put code here for error */
	}

	getMicroTime( &utime );
	syslog( LOG_INFO, "Enter: %sTime=%d: Output.ePrint.print calledFrom: %s", ATTR_probeName_str, utime, ATTR_from_str );
	...
	cCall_print( str );
	getMicroTime( &utime );
	syslog( LOG_INFO, "Leave: %sTime=%d: Output.ePrint.print", ATTR_probeName_str, utime );
}
