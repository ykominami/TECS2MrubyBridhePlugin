...
#endif /* __cplusplus */
/* セル INIB 型宣言 #_CIP_# */
typedef const struct tag_tUnmarshaler_sSample_INIB {
    /* call port #_TCP_# */
    int cServerCall;
    /* call port #_NEP_# */ 
}  tUnmarshaler_sSample_INIB;

/* CB は存在しない。INIB を CB の代わりに使用するための define #_DCI_# */
#define tUnmarshaler_sSample_CB_tab           tUnmarshaler_sSample_INIB_tab
#define tUnmarshaler_sSample_SINGLE_CELL_CB   tUnmarshaler_sSample_SINGLE_CELL_INIB
#define tUnmarshaler_sSample_CB               tUnmarshaler_sSample_INIB
#define tag_tUnmarshaler_sSample_CB           tag_tUnmarshaler_sSample_INIB

extern tUnmarshaler_sSample_CB  tUnmarshaler_sSample_CB_tab[];

/* セルタイプのIDX型 #_CTIX_# */
typedef const struct tag_tUnmarshaler_sSample_INIB *tUnmarshaler_sSample_IDX;
...
#define tUnmarshaler_sSample_N_CELL        (2)  /* セルの個数  #_NCEL_# */
...
/* セルCBを得るマクロ #_GCB_# */
#define tUnmarshaler_sSample_GET_CELLCB(idx) (idx)
 /* 呼び口関数マクロ #_CPM_# */
...
#define tUnmarshaler_sSample_cServerCall_returnGiftFromYou( p_that, buf, len ) \
	  tSample_eEnt_returnGiftFromYou( \
	   (tSample_IDX)0, (buf), (len) )
#define tUnmarshaler_sSample_cServerCall_oneway( p_that, buf, len ) \
	  tSample_eEnt_oneway( \
	   (tSample_IDX)0, (buf), (len) )
#define tUnmarshaler_sSample_cServerCall_shutdown( p_that ) \
	  tSample_eEnt_shutdown( \
	   (tSample_IDX)0 )
...
#define cServerCall_oneway( buf, len ) \
          ((void)p_cellcb, tUnmarshaler_sSample_cServerCall_oneway( p_cellcb, buf, len ))
#define cServerCall_shutdown( ) \
          ((void)p_cellcb, tUnmarshaler_sSample_cServerCall_shutdown( p_cellcb ))
#define cTDR_reset( ) \
...
#define FOREACH_CELL(i,p_cb)   \
    for( (i) = 0; (i) < tUnmarshaler_sSample_N_CELL; (i)++ ){ \
       //(p_cb) = &tUnmarshaler_sSample_CB_tab[i];
